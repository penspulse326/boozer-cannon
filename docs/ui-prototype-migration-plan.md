...
- **來源檔案**：`concept/ui-prototype.html`（Gemini Canvas 生成之調酒酒譜查詢與 ABV 運算引擎原型，原生以 Tailwind CSS 構建）。
- **目標位置**：`apps/web`（Nuxt 4 + Tailwind CSS v4 前端專案）。
- **實作原則**：
  - **樣式庫**：全面採用 **Tailwind CSS v4**（移除 Bootstrap 5 與 SCSS 依賴）。原型本身即以 Tailwind CSS 打造，採用 Tailwind 4 可 100% 無縫保留 Speakeasy 暗色酒吧視覺風格與實用類別（Utility Classes），大幅降低 class 轉換成本並提升渲染效能。
...
- **`feat`**：新增功能或介面特性（**包含 UI 樣式、主題與 Tailwind CSS 配置**）。
- **`refactor`**：既有功能或樣式架構重構（未新增功能或修復 bug）。
- **`fix`**：修復功能瑕疵或樣式跑版 bug。
- **`test`**：新增或修改測試檔案。
- **`style`**：**僅限程式碼格式風格**（如縮排、空白、分號、eslint/prettier 格式化），**不可**用於代表 CSS/UI 樣式。
- **`docs`**：文件修改或說明文件建立。
- **`chore`**：套件依賴變更（如移除 Bootstrap、安裝 Tailwind 4 相關套件）或建置設定調整。
...
```mermaid
flowchart TD
    S1["階段 1: 移除 Bootstrap 並配置 Tailwind 4<br/>feat(web)"] --> S2["階段 2: 字典資料與 ABV 演算引擎<br/>feat(web)"]
...
- **檔案變動**：
  - `apps/web/package.json`：
    - 移除依賴：`bootstrap`、`sass-embedded`。
    - 新增依賴：`tailwindcss`（v4）、`@tailwindcss/vite`。
  - `apps/web/nuxt.config.ts`：
    - 引入 `@tailwindcss/vite` Vite plugin。
    - 載入 Google Fonts（Inter, Playfair Display）與 Font Awesome 6 CDN。
    - 樣式路徑指向 `~/assets/css/main.css`，並移除 SCSS 相關之 preprocessorOptions 配置。
  - `apps/web/app/assets/css/main.css`：
    - 使用 Tailwind 4 的 `@import "tailwindcss";`。
    - 透過 `@theme` 定義 `speakeasy` 專屬調色板（950: `#0a0c10`, 900: `#11141c`, 850: `#171b26`, 800: `#1f2433`, 700: `#2e364a`）與琥珀金（`amber-450`: `#f59e0b`），以及字型家族（Inter、Playfair Display）。
    - 移植原型中的精緻暗色滾動條（`::-webkit-scrollbar`）與 `.scrollbar-none` 工具樣式。
  - 移除舊有檔案：`apps/web/app/assets/scss/main.scss`。
  - `apps/web/app/app.vue`：清理既有 Bootstrap 容器卡片占位程式碼，改為基礎深色容器結構。
- **預計 Commit**：
  ```bash
  feat(web): remove bootstrap and configure tailwind 4 with speakeasy dark theme
  ```

---

### [x] 階段 2：標準字典庫（Taxonomy）與 ABV 融水物理計算引擎
...
### [x] 階段 3：頁面路由架構、頂部導覽列與 Hero 區塊

- **目標**：打通 Nuxt 4 頁面路由架構，以 Tailwind 4 實作全域深色容器、頂部導覽列與 Hero 橫幅。
- **檔案變動**：
  - `apps/web/app/app.vue`：渲染 `<NuxtPage />` 與全域深色容器（`bg-speakeasy-950 text-slate-100 font-sans min-h-screen selection:bg-amber-500 selection:text-speakeasy-950`）。
  - `apps/web/app/pages/index.vue`：建立首頁骨架。
  - 頂部導覽列：BarCraft Logo、全域搜尋列（含清空按鈕與同義詞即時擴展）、我的收藏切換按鈕、新增酒譜觸發按鈕（採用 Tailwind `sticky top-0 z-40 bg-speakeasy-950/80 backdrop-blur-md border-b border-white/10`）。
  - Hero 區塊：品牌標語、ABV 引擎簡介與沉浸式放射漸層背景。
- **預計 Commit**：
  ```bash
...
### [x] 階段 5：酒譜卡片網格展示、互動按鈕與空狀態

- **目標**：將酒譜以 Tailwind CSS Grid 響應式網格展示，支援書籤收藏與按讚。
- **檔案變動**：
  - `apps/web/app/pages/index.vue`：
    - 響應式卡片網格（採用 Tailwind `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6` 排版）。
    - 卡片元件：暗色玻璃質感卡片（`bg-speakeasy-900/70 border border-white/5 hover:border-amber-500/40 transition-all`）、酒圖封面、基酒標籤、預估 ABV 徽章（依酒精濃度變換色彩）、收藏按鈕、中英文品名、風味膠囊（點擊可反向過濾）、指定品牌配方標籤、愛心按讚數。
...
- **檔案變動**：
  - `apps/web/app/pages/index.vue`：
    - 以 Vue `<Teleport to="body">` 與 `<Transition>` 結合 Tailwind CSS（`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm`）實作輕量原生詳情彈窗。
...
- **檔案變動**：
  - `apps/web/app/pages/index.vue`：
    ...
    - Toast 浮動操作提示元件：Tailwind 樣式與動畫（成功發布、收藏狀態切換等）。
- **預計 Commit**：
  ```bash
...
- **工作項目**：
  ...
  - 執行並通過所有檢查指令：
    ...
    - `pnpm run lint:style`（Stylelint - 確保對 Tailwind 4 之 `@import "tailwindcss";` 與 `@theme` 規則相容無報錯）
...
### [x] 階段 10：Layout 系統架構與多頁面架構演進 (Layout Migration & Multi-page Readiness)

- **目標**：將全域導覽、頁尾與通知自 `index.vue` 剝離至 Nuxt Layout 系統，支援未來多頁面（酒譜詳情、工具試算、身分驗證等）之獨立排版需求。
- **工作項目**：
  - [x] **10.1 全域動作與導覽狀態解耦**：建立 `useAddRecipeModal`，並升級 `useToast`、`useRecipeStore`、`useRecipeFilters` 與 `the-header.vue`，支援全域彈窗與狀態共享。（Commit: `36b14d3`）
  - [x] **10.2 實作預設佈局 (`layouts/default.vue`)**：建立 `default.vue`，在 `app.vue` 掛載 `<NuxtLayout>`，並徹底瘦身 `index.vue`（移除 Header、Footer、Toast、AddModal）。（Commit: `b055bfc`）
  - [x] **10.3 實作身分驗證專用佈局 (`layouts/auth.vue`)**：建立極簡 auth layout、登入頁 `app/pages/login.vue`，並於 Header 整合登入連結。（Commit: `143bfff`）
  - [x] **10.4 整合測試與建置檢查**：新增 `apps/web/test/layouts.spec.ts` 驗證 layout 槽位渲染，執行 `pnpm lint`, `pnpm test`, `pnpm --filter web build` 確認全部通過。
